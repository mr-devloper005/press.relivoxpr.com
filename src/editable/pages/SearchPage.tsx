...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden border border-black/10 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-white text-black">
        <section className="mx-auto max-w-[1140px] bg-white">
          <div className="grid bg-white md:grid-cols-[0.8fr_1.2fr]">
            <div>
              <div className="h-full bg-[var(--slot4-accent)] p-7 text-white sm:p-10 lg:p-14">
                ...
                <h1 className="editorial-brand mt-5 text-5xl font-black leading-tight sm:text-7xl">{pagesContent.search.hero.title}</h1>
            ...
            <form action="/search" className="self-center p-6 sm:p-10 lg:p-14">
              ...
              <label className="flex items-center gap-3 border border-black/15 bg-white px-4 py-3 shadow-sm">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded bg-[var(--slot4-accent)] px-6 text-xs font-black uppercase tracking-[0.18em] text-white transition hover:bg-[var(--slot4-deep)]" type="submit">Search</button>
            </form>
          </div>

          <div className="flex flex-wrap items-end justify-between gap-4 px-4 py-10 sm:px-6 lg:px-0">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.24em] opacity-50">{results.length} results</p>
              <h2 className="editorial-brand mt-2 text-4xl font-black text-[var(--slot4-deep)]">{query ? `Results for "${query}"` : pagesContent.search.resultsTitle}</h2>
            </div>
            <Link href="/article" className="inline-flex items-center gap-2 rounded bg-[var(--slot4-accent)] px-5 py-3 text-xs font-black uppercase text-white">Browse latest <ArrowRight className="h-4 w-4" /></Link>
          </div>

          {results.length ? (
            <div className="grid gap-5 px-4 pb-16 sm:px-6 md:grid-cols-2 lg:px-0 xl:grid-cols-3">
