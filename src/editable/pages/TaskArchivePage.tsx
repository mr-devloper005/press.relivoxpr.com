...
function EditorialArchive({
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-white text-[#071b28]">
        <section className="border-b border-black bg-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[var(--slot4-accent)]">Media distribution</p>
            ...
            <p className="max-w-md border-l-4 border-[var(--slot4-accent)] pl-5 text-sm font-bold leading-7 text-black/65">
              Public updates, announcements, and media-ready stories organized for fast discovery.
            </p>
          </div>
        </section>

        <section className="border-b border-black bg-[var(--slot4-accent)] text-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] gap-7 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.16em] sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'text-[var(--slot4-gold)]' : 'hover:text-[var(--slot4-gold)]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'text-[var(--slot4-gold)]' : 'whitespace-nowrap hover:text-[var(--slot4-gold)]'}>
                {item.name}
              </Link>
            ))}
          </div>
          ...
          <section className="mx-auto grid max-w-[var(--editable-container)] border-x border-black bg-white lg:grid-cols-[1.75fr_0.75fr]">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden border-b border-black lg:border-b-0 lg:border-r">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="bg-[var(--slot4-gold)] px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-black">{getCategory(lead, label)}</span>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] border-x border-black/15 bg-[#f5f8fa] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
